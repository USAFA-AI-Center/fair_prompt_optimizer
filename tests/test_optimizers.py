...
class TestAgentModule:
    ...
    def test_init_custom_fields(self):
        ...
        module = AgentModule(mock_agent, input_field="query", output_field="answer")
...
class TestAgentOptimizer:
    ...
    def test_init_with_existing_config(self):
        ...
        mock_agent = Mock()
        existing_config = OptimizedConfig(
            config={
                "version": "1.0",
                "type": "agent",
...
class TestMultiAgentOptimizer:
    ...
    def test_init_with_optimize_workers(self):
        ...
        with patch("fair_prompt_optimizer.config.extract_multi_agent_config") as mock_extract:
            ...
            assert optimizer.optimize_manager
            assert optimizer.optimize_workers
    ...
    def test_compile_with_worker_training_examples(self):
        ...
        with patch("fair_prompt_optimizer.config.extract_multi_agent_config") as mock_extract:
            ...
            mock_worker_configs = {
                "DataGatherer": {
                    "prompts": {"role_definition": "Optimized DG", "examples": ["ex1"]}
                },
                "Summarizer": {"prompts": {"role_definition": "Optimized S", "examples": ["ex2"]}},
            }
            with patch.object(
                optimizer, "_optimize_workers", return_value=mock_worker_configs
            ) as mock_opt_workers:
    ...
    def test_compile_skips_workers_without_training_data(self):
        ...
        manager_examples = [
            TrainingExample(
                inputs={"user_input": "Test"}, expected_output="Result", full_trace="Trace..."
            ),
        ]
        ...
        with patch("fair_prompt_optimizer.config.extract_multi_agent_config") as mock_extract:
            ...
            with patch.object(
                optimizer, "_optimize_workers", side_effect=tracking_optimize_workers
            ):
                result = optimizer.compile(
                    training_examples=manager_examples,
                    metric=mock_metric,
    ...
    def test_compile_with_worker_specific_metrics(self):
        ...
        with patch("fair_prompt_optimizer.config.extract_multi_agent_config") as mock_extract:
            ...
            def capture_optimize_workers(
                worker_training_examples, worker_metrics, default_metric, **kwargs
            ):
    ...
    def test_compile_records_workers_optimized_in_provenance(self):
        ...
        with patch("fair_prompt_optimizer.config.extract_multi_agent_config") as mock_extract:
            ...
            mock_worker_configs = {"DataGatherer": {"prompts": {"role_definition": "Optimized"}}}
            ...
            # Check provenance
            assert result.optimization.optimized
...
class TestOptimizerConfigIntegration:
    ...
    def test_config_examples_accessor(self):
        ...
        config = OptimizedConfig(config={"prompts": {"examples": ["ex1", "ex2"]}})
    ...
    def test_config_role_definition_accessor(self):
        ...
        config = OptimizedConfig(config={"prompts": {"role_definition": "Original"}})
    ...
    def test_provenance_recorded_after_optimization(self):
        ...
        config = OptimizedConfig(config={"prompts": {"examples": []}})
...
class TestErrorHandling:
    ...
    def test_unknown_optimizer_raises(self):
        ...
        with patch.object(OptimizedConfig, "from_agent") as mock_from_agent:
            mock_from_agent.return_value = OptimizedConfig(
                config={"version": "1.0", "type": "agent", "prompts": {}}
            )
            ...
            with pytest.raises(ValueError, match="Unknown optimizer"):
                optimizer.compile(
                    training_examples=[
                        TrainingExample(inputs={"user_input": "test"}, expected_output="test")
                    ],
...
class TestParseOptimizedPrompt:
    ...
    def test_parse_falls_back_to_original(self):
        ...
        assert not result.role_definition_changed
        assert result.format_instructions == ["Original format"]
        assert not result.format_instructions_changed
...
class TestOptimizedPromptsDataclass:
    ...
    def test_default_values(self):
        ...
        assert not prompts.role_definition_changed
        assert prompts.format_instructions is None
        assert not prompts.format_instructions_changed
